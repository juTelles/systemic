...
function BoardB({ roomState, selectedDecisionUIId, handleDecisionSubmit, isPreGame }) {
  const boardRef = useRef(null);
  const nodeRefs = useRef({});
  const [lines, setLines] = useState([]);

  const nodes = useMemo(
    () => (isPreGame ? componentsDefinitions.nodes : roomState?.components?.nodes ?? {}),
  ...
  useEffect(() => {
    // Recalcula quando nodes/decisions mudam (evita linhas "quebradas" após updates)
    requestAnimationFrame(() => computeLines());
        ...
        {Object.entries(nodes).map(([nodeId, node]) => (
          <div key={nodeId} className={styles[nodeId]}>
            <ComponentNode
              ...
              isDisabled={isBoardNodeDisabled(nodeId, availableChosen, roomState)}
              handleDecisionSubmit={() => handleDecisionSubmit(decisionUI, node)}
            />
          </div>
        ))}
      </div>
