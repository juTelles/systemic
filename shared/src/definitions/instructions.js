export const instructions = (state) => {
  ...
  const playerNickname =
    state?.players?.find((p) => p.id === state.flow.currentPlayerId)?.nickname ||
    '';
  ...
  return {
    ...
    ROUND_START: {
      ...
      description: {
        ...
        en: `Start of turn! The team receives ${playerPerRound} "Task Points" to be divided equally among the players to spend on Technical Decisions.`,
      },
    },
    TURN_START: {
      title: 'Player Turn',
      description: {
        pt: `É a vez do jogador ${playerNickname}! Gaste seus pontos em uma das possíveis decisões no painel de decisões!`,
        en: `It's ${playerNickname}'s turn! Spend your points on one of the possible decisions in the decision panel!`,
      },
    },
    AWAIT_DECISION: {
      title: 'Valid Decisions',
      description: {
        pt: `${playerNickname} você possui Pontos de Tarefa para gastar. Escolha uma Decisão Técnica para resolver bugs, desenvolver testes, guardar pontos ou doá-los a outro jogador. Ao final do turno, os pontos que continuarem na mão e não forem guardados ou doados serão descartados. Cada jogador pode ter no máximo ${maxPlayerPoints} pontos no total (mão + banco).`,
    ...
    AWAIT_CARD_DRAW: {
      title: 'Draw Card',
      description: {
        pt: `${playerNickname}, você não pode realizar mais decisões neste turno.\n Agora, clique no deque para comprar uma carta! As cartas podem ser de três tipos: Pontos, Bugs ou Eventos, e cada uma possui um efeito diferente.`,
        en: `${playerNickname} you cannot make more decisions this turn.\n Now, click on the deck to draw a card! Cards can be of three types: Points, Bugs, or Events, and each has a different effect.`,
      },
    },
    PROCESSING_CARD: {
      title: 'Card Effects',
      description: {
        pt: `${playerNickname} você comprou uma carta de ${cardType}! ${cardDescription}`,
        en: `${playerNickname} you drew a ${cardType} card! ${cardDescription}`,
      },
    },
  };
};
